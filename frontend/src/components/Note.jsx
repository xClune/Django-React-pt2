...
function Note({note, onDelete, z}) {
    ...
    return (
        <div className={`note-container text-white flex flex-col w-full border-double border-4 border-stone-400 bg-stone-500 rounded-lg p-5 self-center z-${z} -mt-28 peer peer-hover:translate-y-28 transition-all ease-in-out duration-500`}>
            ...
            <button className={`note-edit ${hidden} self-center text-xs text-white bg-stone-700 hover:bg-stone-800 focus:ring-4 focus:outline-none focus:ring-stone-300 font-medium rounded-lg mt-2 px-1 py-1 text-center dark:bg-stone-600 dark:hover:bg-blue-600 dark:focus:ring-stone-800 z-${z} transition-all ease-in duration-300 mb-2`}>Edit Workout</button>
